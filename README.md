...
#### Почта (verify-email / сброс пароля)

- **DEV (по умолчанию):** `EMAIL_SMTP_HOST` пустой. `send_email()` ничего не
  шлёт наружу — печатает письмо в stderr (console backend), а баннер «почта не
  подтверждена» рисует прямую verify-ссылку прямо в UI. Реальный почтовый ящик и
  SMTP не нужны — этого достаточно для локальной разработки и записи демо.
- **Реальная отправка:** задай `EMAIL_SMTP_HOST` (Gmail/Yandex/Mail.ru/Mailtrap),
  `EMAIL_SMTP_PORT` (587 = STARTTLS), `EMAIL_SMTP_USER`/`EMAIL_SMTP_PASSWORD`.
  Для Gmail/Yandex/Mail.ru пароль — это **пароль приложения**, не пароль от ящика.
- `APP_BASE_URL` должен совпадать с адресом запущенного uvicorn (по умолчанию
  `http://127.0.0.1:8080`) — иначе ссылки в письмах ведут не туда.
